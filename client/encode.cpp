 ...
 * Static Functions
 * */
QByteArray QAESEncryption::Decrypt( QString Cipher_Text,QByteArray key,QByteArray iv)
{
    const std::string cipherTextHex = Cipher_Text.trimmed().toStdString().c_str();
    ...
    QByteArray encodedText = QByteArray::fromStdString(  cipherText  );
    QByteArray decodedText = encryption.decode(encodedText, key.data(),iv.data());

    /**
     * 未知原因末尾会多出了一截，必须将其截取掉...
     **/
    int first= decodedText.lastIndexOf("}");
    //decodedText.left( decodedText.length() - first - 1 );
    decodedText = decodedText.left(  first + 1 );

    //qDebug() << "decode:"<< decodedText;
